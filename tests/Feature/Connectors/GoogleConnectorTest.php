...
function googleCredential(array $attrs = []): SocialToken
{
    return SocialToken::create(array_merge([
        'provider' => 'google',
        'provider_holder_id' => 'g-'.uniqid(),
...
it('renews without returning a new refresh token', function () {
    ...
    $result = google()->refreshCredential(googleCredential());
...
it('is terminal without a refresh token', function () {
    ...
    expect(google()->refreshCredential(googleCredential(['refresh_token' => null]))->outcome)->toBe(RenewalOutcome::Terminal);
...
it('maps invalid_grant to terminal', function () {
    ...
    expect(google()->refreshCredential(googleCredential())->outcome)->toBe(RenewalOutcome::Terminal);
});
...
it('revokes using the refresh token when present', function () {
    ...
    google()->revoke(googleCredential(['access_token' => 'a', 'refresh_token' => 'r']));
